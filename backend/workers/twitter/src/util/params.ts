...
type TwitterAuthType = z.infer<typeof TwitterAuthSchema>;
...
export default function parseArguments<T>(schema: ZodSchema): T {
	const data: Record<string, string | TwitterAuthType> = {};
	let ret: T;
	...
	try {
		if (data.auth) {
			const auth = TwitterAuthSchema.parse(JSON.parse(data.auth as string));
			data.auth = auth;
		}
