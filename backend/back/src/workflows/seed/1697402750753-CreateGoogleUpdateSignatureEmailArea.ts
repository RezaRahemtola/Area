...
export class CreateGoogleUpdateSignatureEmailArea1697402750753 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
       ...
       VALUES ('update-signature-email', 'google', false, 'Update the signature of your Gmail account', $1)`,
       ...
       VALUES ('update-signature-email', 'google', 'https://www.googleapis.com/auth/gmail.settings.basic', 'google')`,
		);
	}

	public async down(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.manager.getRepository(Area).delete({
			id: In(["update-signature-email"]),
			serviceId: In(["google"]),
		});
		await queryRunner.query(
			`DELETE
       FROM "area"
       WHERE "id" = 'update-signature-email'
