...
export class CreateGoogleDriveFolderArea1697900447428 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
       ...
       VALUES ('create-drive-folder', 'google', false, 'Create a new Google Drive folder', $1)`,
       ...
       VALUES ('create-drive-folder', 'google', 'https://www.googleapis.com/auth/drive', 'google')`,
		);
	}

	public async down(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.manager.getRepository(Area).delete({
			id: In(["create-drive-folder"]),
			serviceId: In(["google"]),
		});
		await queryRunner.query(
			`DELETE
       FROM "area"
       WHERE "id" = 'create-drive-folder'
