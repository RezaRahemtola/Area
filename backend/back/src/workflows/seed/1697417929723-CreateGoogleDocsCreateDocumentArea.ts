...
export class CreateGoogleDocsCreateDocumentArea1697417929723 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
       ...
       VALUES ('create-document-docs', 'google', false, 'Create a new Google Docs', $1)`,
       ...
       VALUES ('create-document-docs', 'google', 'https://www.googleapis.com/auth/documents', 'google')`,
		);
	}

	public async down(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.manager.getRepository(Area).delete({
			id: In(["create-document-docs"]),
			serviceId: In(["google"]),
		});
		await queryRunner.query(
			`DELETE
       FROM "area"
       WHERE "id" = 'create-document-docs'
